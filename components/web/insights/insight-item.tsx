...
const InsightItem = ({ LastPostRef, post }: { LastPostRef: React.LegacyRef<HTMLDivElement>, post: ArticleType }) => {
  ...
  return (
    ...
    <div ref={LastPostRef} className='lg:w-3/5 lg:mx-auto  flex flex-col space-y-4  bg-[#bdb1aa] rounded-3xl p-4'>
      ...
      <div className="flex px-2 justify-evenly space-x-8 w-4/5  mx-auto ">

        <Drawer>
          <DrawerTrigger>

            <FaComment className='cursor-pointer ml-14 transition-all hover:scale-110  size-4 lg:size-5' />
          </DrawerTrigger>
          <Comments articleId={post.id} />
        </Drawer>
        <NewsCardInteractions articleid={post.id} userDisliked={post.usersDisliked} userLiked={post.usersLiked} classname=' lg:text-xl space-x-6 justify-evenly w-full text-md' />
      </div>
    </div>
